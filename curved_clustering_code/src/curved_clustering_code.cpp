...
bool CVC::initialize()
{
    if (ros::ok())
    {
        ...
        centroid_pub = nh_.advertise<curved_clustering_code::centroid_list>("centroids", 10);
...
void CVC::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& input){
    ...
    PublishCentroid(*centroids,cluster_results);
    }
}
...
void CVC::PublishCentroid(std::vector<pcl::PointXYZ> &centroids, std::vector<std::vector<pcl::PointXYZ>> &cluster_results){
    curved_clustering_code::centroid_list centroid_arr;
    for (int i = 1 ; i < centroids.size() ; i++){
        curved_clustering_code::centroid centroid_msg; // Publish 하기 위해서 msg 선언 (위에 있는 &centroids는 실제로 위에서 계산한 Centroid * 혼동하지 않기)
        centroid_msg.id = i;
        centroid_msg.x = centroids[i].x;
        centroid_msg.y = centroids[i].y;
        centroid_msg.z = centroids[i].z;
        centroid_msg.radius = 1.0;
        centroid_msg.num = cluster_results[i].size();
